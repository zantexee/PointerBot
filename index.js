...
async function databaseInsert(userId, serverId, startingPoints) {
  connection.handle.query(
    "INSERT INTO `points`(`id`, `userId`, `serverId`, `userPoints`) VALUES (NULL, '" +
      userId +
      "','" +
      serverId +
      "','" +
...
async function databaseUpdate(userId, serverId, points) {
  connection.handle.query(
    'UPDATE `points` SET `userPoints` = (`userPoints` + ?) WHERE userid = ? AND serverId = ?',
    [points, userId, serverId],
    function (err, res) {
      if (err) console.log(err);
    },
  );
...
function getPoints(userid, serverId) {
  connection.handle.query(
    'SELECT `userPoints` FROM `points` WHERE `userId` = ' +
      userId +
      ' AND `serverId` = ' +
      serverId,
...
function databaseDelete(serverId) {
  connection.handle.query(
    "DELETE FROM points WHERE serverId='" + server + "'",
    function (err, res) {
      if (err) console.log(err);
    },
  );
...
client.on('message', (msg) => {
  ...
  if (commandNoPrefix.search('givep') != -1) {
    if (args[0] === undefined || args[1] === undefined) {
      msg.channel.send(
        'Syntax: ' +
          prefix +
          'givep {number of credits} {username/mention of the user}',
      );
      return 1;
    }
    ...
    if (userNameArgs.search('@') != -1) {
      if (userNameArgs.indexOf('everyone') !== -1) {
        ...
        for (let i = 0; i < userL.length; i++) {
          connection.handle.query(
            'SELECT `id` FROM `points` WHERE `userId` = ?',
            [userL[i]],
            ' AND `serverId` = ?',
            [server],
            function (err, res, row) {
  ...
  if (commandNoPrefix.search('redeem') != -1) {
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        'The multiplier multiplies the points needed for redeeming a role.',
      )
      ...
      function (err, res, row) {
        ...
        if (res.length != 0) {
          ...
          role = msg.guild.roles.cache.find(
            (role) => role.name === RANKS[rank],
          );
          roleName = role.name;
          msg.member.roles.add(role.id);
          msg.author.send(
            `You have redeemed your ${roleName} role on the ${guildName} server.`,
          );
        } else {
          msg.author.send(embed);
          msg.author.send(
            `There has been an error redeeming your role. Please check your points and try again later.`,
          );
        }
      },
    );
